...
void Canvas_controller::create_image() {
  sf::Drawable *new_shape = image_factory->createShape();
  shapes.push_back(new_shape);
  canvas_view->render(shapes);
}
...
void Canvas_controller::select_shape(const sf::Vector2f &click_position) {
  for (auto &shape : shapes) {
    if (auto circle_shape = dynamic_cast<sf::CircleShape *>(shape)) {
      sf::Vector2f center =
          circle_shape->getPosition() +
          sf::Vector2f(circle_shape->getRadius(), circle_shape->getRadius());
      float dx = click_position.x - center.x;
      float dy = click_position.y - center.y;
      if ((dx * dx + dy * dy) <=
          circle_shape->getRadius() * circle_shape->getRadius()) {
...
void Canvas_controller::move_shape(const sf::Vector2f &new_position) {
  ...
  } else if (auto circle_shape =
                 dynamic_cast<sf::CircleShape *>(selected_shape)) {
...
// 드래그 종료
void Canvas_controller::end_drag() { is_selected = false; }
