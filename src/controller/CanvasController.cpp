...
void CanvasController::select_shape(const sf::Vector2f &click_position, bool multiSelect)
{
  ...
  for (auto &shape : shapes)
  {
    if (shape->isSelected(click_position))
    {
      ...
      auto highlightCommand = std::make_shared<HighlightCommand>(selected_shape, true);
      executeCommand(highlightCommand);
...
void CanvasController::move_shape(const sf::Vector2f &new_position)
{
  if (selected_shape)
  {
    auto moveCommand = std::make_shared<MoveCommand>(selected_shape, new_position);
...
void CanvasController::resize_shape(const sf::Vector2f &new_size)
{
  if (selected_shape)
  {
    auto resizeCommand = std::make_shared<ResizeCommand>(selected_shape, new_size);
...
{
  return selected_shape && std::dynamic_pointer_cast<CompositeShape>(selected_shape) == nullptr;
}
