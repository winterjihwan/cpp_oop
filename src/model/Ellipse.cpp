...
// get_width() 정의
float Ellipse::get_width() const
{
    return ellipse.getRadius() * 2.0f;
}
...
{
    sf::Vector2f center = ellipse.getPosition() + sf::Vector2f(ellipse.getRadius(), ellipse.getRadius());
...
// 위치 반환 메서드 정의
sf::Vector2f Ellipse::getPosition() const
{
...
{
    return sf::Vector2f(ellipse.getRadius() * 2.0f, ellipse.getRadius() * 2.0f * ellipse.getScale().y);
}
